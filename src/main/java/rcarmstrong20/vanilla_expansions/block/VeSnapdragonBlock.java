...
public class VeSnapdragonBlock extends FlowerBlock
{
	...
	private boolean isValidBlock(IBlockReader world, BlockPos pos)
	{
		...
		{
			return VeBlockTags.END_PLANTABLE.func_230235_a_(block) && VeBlockTags.END_POTTABLE.func_230235_a_(this.getBlock());
		}
		else
		{
			return VeBlockTags.END_PLANTABLE.func_230235_a_(block);
		}
	}
		...
		if(!world.isRemote && entity instanceof PlayerEntity)
		{
	        ...
	        if(!playerEntity.isSneaking() && playerEntity.getMotion().getY() == 0.0D)
	        {
	        	for(int i = 0; i < 16; ++i)
	        	{
	        		...
	        		double newY = MathHelper.clamp(playerEntity.getPosY() + (double)(playerEntity.getRNG().nextInt(16) - 8), 0.0D, (double)(world.func_234938_ad_() - 1));
