...
public class VeCollisionUtil
{
    ...
    public static VoxelShape rotate(@Nullable Axis axis, double radians, VoxelShape shape)
    {
        ...
        {
            Pair<Double, Double> min = axis == Axis.X ? rotatePoint(box.minY, box.minZ, radians)
                    : (axis == Axis.Z ? rotatePoint(box.minX, box.minY, radians)
                            : rotatePoint(box.minX, box.minZ, radians));
            Pair<Double, Double> max = axis == Axis.X ? rotatePoint(box.maxY, box.maxZ, radians)
                    : (axis == Axis.Z ? rotatePoint(box.maxX, box.maxY, radians)
                            : rotatePoint(box.maxX, box.maxZ, radians));
            collision = VoxelShapes.or(collision,
                    axis == Axis.X
                            ? VoxelShapes.create(box.minX, min.getFirst(), min.getSecond(), box.maxX, max.getFirst(),
                                    max.getSecond())
                            : (axis == Axis.Z
                                    ? VoxelShapes.create(min.getFirst(), min.getSecond(), box.minZ, max.getFirst(),
                                            max.getSecond(), box.maxZ)
                                    : VoxelShapes.create(min.getFirst(), box.minY, min.getSecond(), max.getFirst(),
                                            box.maxY, max.getSecond())));
        }
        return collision;
    }
