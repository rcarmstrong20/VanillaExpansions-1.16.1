...
public class VeEaselScreen extends ContainerScreen<VeEaselContainer>
{
	...
	@Override
	public void func_230430_a_(MatrixStack matrixStack, int p_230430_2_, int p_230430_3_, float p_230430_4_) //Same as render
	{
	 ...
	 */
	@Override
	protected void func_230450_a_(MatrixStack matrixStack, float partialTicks, int mouseX, int mouseY) //Same as drawGuiContainerBackgroundLayer
	{
		...
		this.func_238474_b_(matrixStack, startXPos, startYPos, 0, 0, this.xSize, this.ySize); //Same as blit, render background
		int sliderProgressPos = (int)(41.0F * this.sliderProgress);
		this.func_238474_b_(matrixStack, startXPos + 119, startYPos + 13 + sliderProgressPos, 232 + (this.canScroll() ? 0 : 12), 0, 12, 15); //Same as blit, render scroll wheel
	 ...
	 */
	private void drawRecipesBackground(MatrixStack matrixStack, int mouseX, int mouseY, int left, int top, int recipeIndexOffsetMax)
	{
		...
		{
			this.func_238474_b_(matrixStack, guiXPos + paperSlot.xPos, guiYPos + paperSlot.yPos, this.xSize, 0, 16, 16); //Same as blit
		}
		
		//Draw the first dye slot icon when the slot is empty
		if (!dyeSlot1.getHasStack())
		{
			this.func_238474_b_(matrixStack, guiXPos + dyeSlot1.xPos, guiYPos + dyeSlot1.yPos, this.xSize + 16, 0, 16, 16); //Same as blit
		}
		
		//Draw the second dye slot icon when the slot is empty
		if (!dyeSlot2.getHasStack())
		{
			this.func_238474_b_(matrixStack, guiXPos + dyeSlot2.xPos, guiYPos + dyeSlot2.yPos, this.xSize + 16, 0, 16, 16); //Same as blit
		}
	 ...
	 */
	@Override
	public boolean func_231044_a_(double p_mouseClicked_1_, double p_mouseClicked_3_, int p_mouseClicked_5_) //Same as mouseClicked
	{
		...
		if (this.hasItemsInInputSlot)
		{
			...
			for(int l = this.recipeIndexOffset; l < k; ++l)
			{
				...
				if (d0 >= 0.0D && d1 >= 0.0D && d0 < 14.0D && d1 < 14.0D && this.container.enchantItem(this.getMinecraft().player, l))
				{
	 ...
	 */
	@Override
	public boolean func_231045_a_(double p_mouseDragged_1_, double p_mouseDragged_3_, int p_mouseDragged_5_, double p_mouseDragged_6_, double p_mouseDragged_8_) //Same as mouseDragged
	{
