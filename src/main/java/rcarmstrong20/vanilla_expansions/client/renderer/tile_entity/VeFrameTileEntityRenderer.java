...
public class VeFrameTileEntityRenderer extends TileEntityRenderer<VeFrameTileEntity>
{
    ...
    @Override
    public void render(VeFrameTileEntity tileEntityIn, float partialTicks, MatrixStack matrixStackIn,
            IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn)
    {
        ...
        if (itemStack != ItemStack.EMPTY)
        {
            ...
            {
                renderPainting(0.5D, 0.5D, 0.9D, 0F, itemStack, matrixStackIn, bufferIn, combinedLightIn,
                        combinedOverlayIn);
            } else if (direction == Direction.SOUTH)
            {
                renderPainting(0.5D, 0.5D, 0.1D, 0F, itemStack, matrixStackIn, bufferIn, combinedLightIn,
                        combinedOverlayIn);
            } else if (direction == Direction.WEST)
            {
                renderPainting(0.9D, 0.5D, 0.5D, 270F, itemStack, matrixStackIn, bufferIn, combinedLightIn,
                        combinedOverlayIn);
            } else
            {
                renderPainting(0.1D, 0.5D, 0.5D, 270F, itemStack, matrixStackIn, bufferIn, combinedLightIn,
                        combinedOverlayIn);
            }
        }
    }
     ...
     */
    private static void renderPainting(double x, double y, double z, float rotation, ItemStack itemStack,
