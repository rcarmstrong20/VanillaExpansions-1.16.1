...
public class VeEaselContainer extends Container
{
        ...
        this.outputInventorySlot = this.addSlot(new Slot(this.outputInventory, 0, 143, 58)
        {
            ...
            public ItemStack onTake(PlayerEntity thePlayer, ItemStack stack)
            {
                ...
                worldPosCallable.consume((world, pos) ->
                {
                    ...
                    {
                        world.playSound((PlayerEntity) null, pos, SoundEvents.UI_LOOM_TAKE_RESULT, SoundCategory.BLOCKS,
                                1.0F, 1.0F);
    ...
    public ItemStack transferStackInSlot(PlayerEntity playerIn, int index)
    {
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            {
                if (!this.mergeItemStack(itemStack1, paperSlotIndex, paperSlotIndex + 1, false)
                        && this.getPaperInventorySlot().isItemValid(itemStack1))
                {
                    return ItemStack.EMPTY;
                }
                ...
                {
                    // If the slot is empty move the dye the first dye slot or if the dye matches
                    // the dye in the slot merge the item stacks
    ...
    public void onCraftMatrixChanged(IInventory inventoryIn)
    {
        ...
        if (paperItemStack.getItem() != this.itemStackInput0.getItem()
                || dyeItemStack.getItem() != this.itemStackInput1.getItem()
                || dyeItemStack2.getItem() != this.itemStackInput2.getItem())
        {
    ...
    {
        return this.getPaperInventorySlot().getHasStack() && this.getDyeInventorySlot().getHasStack()
                && this.getDyeInventorySlot2().getHasStack() && !this.recipes.isEmpty();
    }
}
